...
const taskGrid: Record<TaskKey, string> = {
  ...
  classified: 'grid gap-6 sm:grid-cols-2 xl:grid-cols-4',
...
const cardBase = 'group block rounded-[8px] border border-[var(--editable-border)] bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_44px_rgba(8,6,22,0.12)]'
  ...
  return (
    <EditableSiteShell>
      <main style={taskThemeStyle(task)} className="min-h-screen bg-white text-[var(--tk-text)]">
        <header className="border-b border-[var(--tk-line)] bg-[linear-gradient(135deg,#f8f9ff_0%,#eef2ff_58%,#ffffff_100%)]">
          <div className="mx-auto max-w-[var(--editable-container)] px-6 py-14 text-center sm:py-16 lg:px-8">
            <div className="inline-flex items-center gap-3 rounded-full bg-white px-4 py-2 text-[11px] font-extrabold uppercase tracking-[0.14em] text-[var(--tk-accent)] shadow-sm">
              <span>{theme.kicker}</span>
              <span className="h-1 w-1 rounded-full bg-[var(--tk-accent)] opacity-50" />
              <span className="text-[var(--tk-muted)]">{label}</span>
            </div>
            <h1 className="editable-display mx-auto mt-6 max-w-4xl text-balance text-[2.35rem] font-bold leading-tight sm:text-5xl lg:text-6xl">
              {voice?.headline || `Browse ${label}`}
            </h1>
            <p className="mx-auto mt-5 max-w-3xl text-base leading-8 text-[var(--tk-muted)]">{voice?.description || theme.note}</p>
            {voice?.chips?.length ? (
              <div className="mt-7 flex flex-wrap justify-center gap-3">
                {voice.chips.map((chip) => (
                  <span key={chip} className="rounded-full border border-[var(--editable-border)] bg-white px-4 py-2 text-sm font-bold text-[#233544] shadow-sm">{chip}</span>
                ))}
              </div>
            ) : null}

            <div className="mx-auto mt-10 flex max-w-5xl flex-col gap-4 rounded-[8px] border border-[var(--tk-line)] bg-white p-4 shadow-[0_18px_50px_rgba(8,6,22,0.08)] sm:flex-row sm:items-center sm:justify-between">
              ...
              <form action={basePath} className="flex items-center gap-2.5">
                <div className="relative">
                  <select
                    ...
                    className="h-11 appearance-none rounded-[6px] border border-[var(--tk-line)] bg-white pl-4 pr-10 text-sm font-semibold text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
                ...
                <button className="inline-flex h-11 items-center rounded-[6px] bg-[var(--tk-accent)] px-5 text-sm font-extrabold text-[var(--tk-on-accent)] transition hover:-translate-y-0.5">Apply</button>
              </form>
            </div>
          </div>
        </header>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-6 py-12 sm:py-16 lg:px-8">
          {posts.length ? (
            ...
            <div className="mx-auto max-w-xl rounded-[4px] border border-dashed border-[var(--tk-line)] bg-white px-8 py-16 text-center shadow-sm">
          ...
          {posts.length ? (
            <nav className="mt-16 flex items-center justify-center gap-3 text-sm">
              {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-[4px] border border-[var(--tk-line)] px-5 py-2.5 font-bold transition hover:border-[var(--tk-accent)]">Previous</Link> : null}
              <span className="rounded-[4px] border border-[var(--tk-line)] bg-white px-5 py-2.5 font-bold text-[var(--tk-muted)]">Page {page} of {pagination.totalPages || 1}</span>
              {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-[4px] border border-[var(--tk-line)] px-5 py-2.5 font-bold transition hover:border-[var(--tk-accent)]">Next</Link> : null}
            </nav>
          ) : null}
        </section>
      </main>
  ...
  return (
    <Link href={href} className={`${cardBase} overflow-hidden`}>
      ...
      <div className="p-6 sm:p-7">
        ...
        <h2 className="editable-display mt-3 text-2xl font-bold leading-tight">{post.title}</h2>
...
function ListingArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex items-center gap-5 p-5 sm:p-6`}>
      <div className="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-[8px] border border-[var(--tk-line)] bg-[var(--tk-raised)]">
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col p-6 sm:p-7`}>
      <div className="flex items-start justify-between gap-4">
        ...
        {condition ? <span className="rounded-full bg-[var(--tk-accent-soft)] px-3 py-1 text-[11px] font-bold uppercase tracking-[0.12em] text-[var(--tk-accent)]">{condition}</span> : null}
      </div>
      <h2 className="editable-display mt-5 text-xl font-bold leading-tight">{post.title}</h2>
      <RatingLine post={post} />
...
function BookmarkArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex gap-4 p-6`}>
      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[8px] bg-[var(--tk-accent-soft)] text-[var(--tk-accent)]">
        <Globe className="h-5 w-5" />
      </div>
      <div className="min-w-0 flex-1">
        <span className="text-[11px] font-bold uppercase tracking-[0.14em] text-[var(--tk-muted)]">Saved · {String(index + 1).padStart(2, '0')}</span>
        <h2 className="editable-display mt-1.5 text-lg font-bold leading-tight">{post.title}</h2>
...
function PdfArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col p-6 sm:p-7`}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex h-12 w-12 items-center justify-center rounded-[8px] bg-[var(--tk-accent-soft)] text-[var(--tk-accent)]"><FileText className="h-6 w-6" /></div>
        <span className="rounded-full border border-[var(--tk-line)] px-3 py-1 text-[11px] font-bold uppercase tracking-[0.12em] text-[var(--tk-muted)]">{category}</span>
      </div>
      <h2 className="editable-display mt-6 text-xl font-bold leading-tight">{post.title}</h2>
      <RatingLine post={post} />
      <p className="mt-3 line-clamp-3 flex-1 text-sm leading-7 text-[var(--tk-muted)]">{getSummary(post)}</p>
